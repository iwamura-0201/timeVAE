...
try:
    import torch
    from vae.timevae_torch import TimeVAE as TimeVAE_Torch
...
def instantiate_vae_model(
...
) -> Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]:
    """
    ...
    Args:
        vae_type (str): The type of VAE model to instantiate.
                        One of ('vae_dense', 'vae_conv', 'timeVAE', 'timeVAE_torch').
        sequence_length (int): The sequence length.
        feature_dim (int): The feature dimension.
        batch_size (int): Batch size for training.

    Returns:
        Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]: The instantiated VAE model.
    ...
    if vae_type == "vae_dense":
    ...
    elif vae_type == "timeVAE_torch":
        if not torch_available:
            raise ImportError("PyTorch is not available. Please install torch to use timeVAE_torch.")
        # PyTorch model doesn't take batch_size in init, but in fit/predict
        vae = TimeVAE_Torch(
            seq_len=sequence_length,
            feat_dim=feature_dim,
            **kwargs,
        ...
        raise ValueError(
            ...
            "Please choose from vae_dense, vae_conv, timeVAE, timeVAE_torch."
        )
...
def train_vae(vae, train_data, max_epochs, verbose=0):
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]): The VAE model to train.
...
def save_vae_model(vae, dir_path: str) -> None:
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]): The VAE model to save.
        dir_path (str): The directory to save the model weights.
    """
    vae.save(dir_path)


def load_vae_model(vae_type: str, dir_path: str) -> Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]:
    """
    ...
    Returns:
        Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]: The loaded VAE model.
    """
    if vae_type == "vae_dense":
        ...
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        vae = TimeVAE_Torch.load(dir_path, device=device)
    else:
        raise ValueError(
            f"Unrecognized model type [{vae_type}]. "
            "Please choose from vae_dense, vae_conv, timeVAE, timeVAE_torch."
...
def get_posterior_samples(vae, data):
    ...
    Args:
        vae (Union[VAE_Dense, VAE_Conv, TimeVAE, TimeVAE_Torch]): The trained VAE model.
